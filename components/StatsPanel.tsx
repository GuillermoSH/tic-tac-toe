...
export function StatsPanel({ winner, isDraw }: StatsPanelProps) {
  ...
  return (
    <View
      className={`p-6 rounded-2xl border shadow-lg ${isDark
          ? "bg-neutral-900 border-neutral-700"
          : "bg-white border-gray-200"
        }`}
    >
      <Text
        className={`text-2xl font-bold mb-6 text-center ${isDark ? "text-neutral-100" : "text-gray-800"
          }`}
      >
      ...
      <View className="flex-row justify-between mb-6">
        ...
        <View className="flex-1 items-center border-r border-neutral-500 pr-3">
          <Text
            className={`text-xl font-bold mt-2 ${isDark ? "text-indigo-400" : "text-blue-500"
              }`}
          >
            Jugador X
          </Text>
          <Text
            className={`text-4xl font-extrabold ${isDark ? "text-indigo-300" : "text-blue-600"
              }`}
          >
            {winsX}
          </Text>
        ...
        <View className="flex-1 items-center pl-3">
          <Text
            className={`text-xl font-bold mt-2 ${isDark ? "text-amber-400" : "text-amber-500"
              }`}
          >
            Jugador O
          </Text>
          <Text
            className={`text-4xl font-extrabold ${isDark ? "text-amber-300" : "text-amber-600"
              }`}
          >
            {winsO}
          </Text>
      ...
      <View className="flex-row justify-between mb-4 px-2">
        <Text
          className={`text-base font-medium ${isDark ? "text-neutral-300" : "text-gray-600"
            }`}
        >
          🤝 Empates: {draws}
        </Text>
